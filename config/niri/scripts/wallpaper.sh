...
# Security Fix: Added '--' to prevent flag injection
awww img \
...
if [ "$pmode" = "manual" ]; then
    ...
    # Security Fix: Added '--' to prevent flag injection into the python script
    python3 "$(dirname "$0")/wallcolors.py" -- "$pic" >/dev/null 2>&1 || true
fi

# Note: hyprctl reload has been removed. Niri monitors its config file and auto-reloads.
